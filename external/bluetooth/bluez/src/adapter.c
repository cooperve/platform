...
//Amp State Change
static void emit_amp_state_changed(const char *path, uint32_t app_id, const char *address, uint32_t amp_state)
{
    DBusMessage *signal;
    DBusMessageIter iter;
    va_list var_args;
    ...
    signal = dbus_message_new_signal(path, ADAPTER_INTERFACE,
					"AmpStateChanged");
    if (!signal) {
        error("Unable to allocate new %s.AmpStateChanged signal",
            ADAPTER_INTERFACE);
       return;
    }
    dbus_message_append_args(signal, DBUS_TYPE_UINT32, &app_id, DBUS_TYPE_STRING, &address, DBUS_TYPE_UINT32, &amp_state, DBUS_TYPE_INVALID);
    g_dbus_send_message(connection, signal);
}
...
void adapter_emit_amp_state_changed(struct btd_adapter *adapter, uint32_t app_id, const bdaddr_t *peer, uint32_t amp_state)
{
    ...
    info("*********************** sending amp state change app_id %d address %s amp_state %d", app_id, paddr, amp_state);
    emit_amp_state_changed(adapter->path, app_id, paddr, amp_state);
}
