...
float CSSStyleSelector::fontSizeForKeyword(Document* document, int keyword, bool fixed)
{
    ...
    // P110720-2433 segmentation fault issue
    int index = keyword - CSSValueXxSmall;
    if (index < 0) {
	index = 0;
    } else if (index >= totalKeywords) {
	index = totalKeywords;
    }

    return max(fontSizeFactors[index]*mediumSize, minLogicalSize);
}
